...
Describe what a venue looks like from a seat: where it is, what it is worth, and
whether a fan can use it.
...
| `CloseSeatBlock` | venue staff   | Command |
...
- A seat exists for a venue, not for an event; an event borrows the venue's map.
- Closing a block of seats never cancels a ticket already issued for them.
...
- Tickets issued for seats closed after the fact.
