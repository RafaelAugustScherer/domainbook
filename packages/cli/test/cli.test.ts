...
describe("what the CLI says when it is misused", () => {
  it("names an unknown command and lists the real ones", () => {
    expect(failed("chekc")).toEqual([
      '"chekc" is not a domainbook command — the commands are validate, init, check, sync, status, hooks, instructions, serve, build, export, and new; run "domainbook --help" to see them',
    ]);
  });
